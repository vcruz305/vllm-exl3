...
def cache_first_groups(groups,resident_keys):
    """Consume resident experts before misses can evict their still-needed bytes."""
    ready,missing=[],[]
    for group in groups:
        hits=[item for item in group if item.key in resident_keys]
        misses=[item for item in group if item.key not in resident_keys]
        if hits:ready.append(hits)
        if misses:missing.append(misses)
