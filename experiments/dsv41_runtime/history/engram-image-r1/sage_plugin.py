...
def get_session():
    ...
    with _lock:
        ...
        if not config.model_config.enforce_eager or config.speculative_config is not None:raise ValueError('Disk candidate requires eager and DSpark off')
        if config.scheduler_config.max_num_seqs!=1 or config.scheduler_config.max_num_batched_tokens>128:raise ValueError('Disk candidate requires C1 and bounded prefill')
