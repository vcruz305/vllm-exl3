...
def install(session):
    ...
    def snapshot():
        ...
        return {'sage_offload': 'telemetry', 'rank': session.rank, 'time': time.time(),
            ...
            'layers': layers, 'engram': engram, 'engram_cache': __import__('engram_prefetch').snapshot(), 'decoder_tail': __import__('decoder_tail').snapshot(), 'kv_layout': __import__('kv_layout_audit').snapshot(), 'cache': dict(cache.stats),
