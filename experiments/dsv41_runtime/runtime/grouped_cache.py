...
class GroupedExpertCache(ExpertCache):
    ...
    def _routing_groups(self, layer, rows):
        return routing_groups(layer, rows, self.store.records, max_routes=self.MAX_ROUTES)

    def _prepare_dispatch(self, groups):
        """Optional I/O planning hook; the synchronous grouped path does none."""
        return None
    ...
    def apply(self, layer, x, ids, weights, *, limit=10.0):
        ...
        with self.lock, torch.cuda.device(self.device):
            ...
            try:
                ...
                groups = self._routing_groups(layer, ids.cpu().tolist())
                dispatch_groups = []
                for group in groups:
                ...
                self._prepare_dispatch(dispatch_groups)
