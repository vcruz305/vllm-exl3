...
def install(session):
    ...
    def snapshot():
        ...
        native = cache.refresh_native_stats()
        return {'sage_offload': 'telemetry', 'rank': session.rank, 'time': time.time(),
            'identity': getattr(session, 'runtime_identity', None),
            'layers': layers, 'engram': engram, 'engram_cache': __import__('engram_prefetch').snapshot(), 'graph_runtime': __import__('graph_runtime').snapshot(), 'decoder_tail': __import__('decoder_tail').snapshot(), 'kv_layout': __import__('kv_layout_audit').snapshot(), 'cache': dict(cache.stats),
            'native_service': native, 'resident_expert_bytes': cache.resident_bytes, 'expert_disk_bytes': session.store.bytes_read + int(native.get('bytes_read',0)),
            'cuda_allocated': torch.cuda.memory_allocated(), 'cuda_reserved': torch.cuda.memory_reserved(),
            'host': host_memory(), 'timing_scope': 'Python apply counters cover eager/warmup/capture only; graph_runtime replay and native service counters cover replay; host timings are unsynchronized'}
    ...
    def apply(layer, *args, **kwargs):
        ...
        if layer == max(session.quotas) and not __import__('torch').cuda.is_current_stream_capturing(): emit()
        return result
