...
void sage_heterogeneous_moe(
    ...
    std::call_once(configured[x.get_device()], [&] {
        ...
        C10_CUDA_CHECK(cudaFuncSetAttribute(sage_heterogeneous_kernel<0,256,2,true>, cudaFuncAttributeMaxDynamicSharedMemorySize, SMEM_MAX));
        int dynamic_active=0;
        C10_CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&dynamic_active,sage_heterogeneous_kernel<0,256,2,true>,threads,SMEM_MAX));
        TORCH_CHECK(dynamic_active>=1,"Dynamic expert barriers require resident blocks");
        int active=0;
    ...
    if (x.size(0)==1 && experts==6) {
    sage_heterogeneous_kernel<0,256,2,true><<<dim3(sms,1,1),threads,SMEM_MAX,at::cuda::getCurrentCUDAStream()>>>(
