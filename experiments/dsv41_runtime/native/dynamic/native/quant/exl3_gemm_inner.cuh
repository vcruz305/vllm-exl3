...
(
    ...
    int size_n_stride = 0, int group_block = -1, int group_width = 0
)
{
    ...
    int num_slices = group_width ? group_width : int(gridDim.x);
    int slice_block = group_block >= 0 ? group_block : int(blockIdx.x);
    int slice_beg = tiles_k * tiles_n * slice_block / num_slices;
    int slice_end = tiles_k * tiles_n * (slice_block + 1) / num_slices;
