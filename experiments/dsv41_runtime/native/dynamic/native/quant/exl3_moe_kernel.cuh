...
template<int t_bits, int MOE_TILESIZE_N, int cb, bool dynamic_groups = false>
__global__ __launch_bounds__(EXL3_GEMM_BASE_THREADS * MOE_TILESIZE_K / 16)
void sage_heterogeneous_kernel(EXL3_MOE_KERNEL_ARGS)
{
    int group_idx, block_idx, group_size, num_groups;
    if constexpr (dynamic_groups) {
        // One flat resident grid. Every block sees identical device counts;
        // no host readback or launch-shape change is required on graph replay.
        int active = 0;
        for (int e = 0; e < num_experts; ++e)
            active += expert_count[e] > 0 && expert_count[e] <= max_tokens_per_expert;
        if (active == 0) return;
        num_groups = MIN(active, concurrency);
        group_size = MIN(int(gridDim.x) / num_groups, MOE_MAX_SMS_PER_EXPERT);
        group_idx = int(blockIdx.x) / group_size;
        block_idx = int(blockIdx.x) % group_size;
        if (group_idx >= num_groups) return;
    } else {
        group_idx = blockIdx.z;
        block_idx = blockIdx.x;
        group_size = gridDim.x;
        num_groups = gridDim.z;
    }
    ...
    for (; expert_idx < num_experts; ++expert_idx)
    {
        ...
        const int K_gate = expert_bits[3 * expert_idx];
        const int K_up = expert_bits[3 * expert_idx + 1];
        const int K_down = expert_bits[3 * expert_idx + 2];
        ...
        auto gemm_up = [&](const half* in_addr, half* out_addr, const uint16_t* trellis, const int K)
        {
            ...
            while (size_m > 0)
            {
                #define ARGS            \
                    ...
                    intermediate_dim,   \
                    locks,              \
                    nullptr, 0, block_idx, group_size
                #define SHAPE_ARGS      \
                    MOE_TILESIZE_M,     \
                    MOE_TILESIZE_K,     \
                    MOE_TILESIZE_N,     \
