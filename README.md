...
On vLLM 0.28.1rc1 nightly with the V2 model runner and Qwen3.8-Flash-Next, prompts of roughly 33 to 144 tokens wedged the engine (EngineCore at 100% CPU, GPU busy at idle power, never recovering) and MTP k=2 decoding wedged after 30 to 60 minutes. exllamav3's non-cooperative GEMV kernels cover at most 2 rows on K=5 layers, so every dense call with 3 to 144 rows ran the cooperative trellis GEMM (grid barriers over a shared lock buffer). Dense rows up to 2 now keep the GEMV; rows 3 to 8 slice through 2-row GEMV chunks; rows 9 to 144 reconstruct and run hgemm (bit-identical, prefill-only). A 4-worker MTP stress that wedged the baseline in 161 s ran clean for the full test window with decode speed unchanged. `VLLM_EXL3_COOP_GEMM=1` restores cooperative GEMM for A/B testing; `VLLM_EXL3_RECONSTRUCT_MIN_ROWS` adjusts the reconstruct threshold. `VLLM_EXL3_PREFILL_SYNC` stays as an optional knob for other timing issues.
